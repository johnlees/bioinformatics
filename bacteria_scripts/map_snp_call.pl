...
else
{
   ...
   # Filter variants
   my $filtered_vcf = mapping::random_string() . "filtered.$output_vcf";
   mapping::filter_vcf($output_vcf, $filtered_vcf, \@vcf_filters, \@vcf_filter_names);

   rename $filtered_vcf, $output_vcf;
   system("bcftools index -f $output_vcf");
