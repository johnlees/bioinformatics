...
# Set up hash of arrays for r^2 vals
my %r2;
...
while (my $gen_line = <IN>)
{
   ...
   if ($pos eq $ref_pos)
   {
      ...
      $linearFit->setData (\@$ref_mac, \@$imputed_mac) or die "Invalid data";

      my $r2;
      if (!defined($r2 = $linearFit->rSquared))
      {
         # Rationale: unimputable values give no association power
         $r2 = 0;
      }

      # Find correct bin, and put in r^2
...
foreach my $maf (sort{$a<=>$b} keys %r2)
{
   # Average r2 and print
   my $avg_r2 = Math::NumberCruncher::Mean(\@{$r2{$maf}});
   print(scalar(@{$r2{$maf}}) . " $maf $avg_r2\n");
   if ($output_file)
   {
      print OUT " $maf $avg_r2\n"
   }
}
